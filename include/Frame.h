...
class FeatureExtractor;    // Add this forward declaration
class ORBextractor;
...
public:
    ...
    Frame(const cv::Mat &imGray, const double &timeStamp, FeatureExtractor* extractor,
    ...
    // Extract features
    void ExtractFeatures(int flag, const cv::Mat &im, const int x0, const int x1);
...
public:
    ...
    // Feature extractors. The right is used only in the stereo case.
...
public:
    ...
    Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, 
          ...
          Sophus::SE3f& Tlr, Frame* pPrevF = static_cast<Frame*>(NULL), 
          const IMU::Calib &ImuCalib = IMU::Calib());
