...
cv::BFMatcher Frame::BFmatcher = cv::BFMatcher(cv::NORM_L2);

Frame::Frame(): mpcpi(NULL), mpImuPreintegrated(NULL), mpPrevFrame(NULL), mpImuPreintegratedFrame(NULL), mpReferenceKF(static_cast<KeyFrame*>(NULL)), 
...
Frame::Frame(const Frame &frame)
    ...
    mpFeatureExtractorLeft(frame.mpFeatureExtractorLeft), mpFeatureExtractorRight(frame.mpFeatureExtractorRight),
...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, 
             FeatureExtractor* extractorLeft, FeatureExtractor* extractorRight, ORBVocabulary* voc, 
             cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, 
             GeometricCamera* pCamera, Frame* pPrevF, const IMU::Calib &ImuCalib)
    :mpcpi(NULL),mpORBvocabulary(voc),
    mpFeatureExtractorLeft(extractorLeft),mpFeatureExtractorRight(extractorRight), 
    mTimeStamp(timeStamp), mK(K.clone()), mK_(Converter::toMatrix3f(K)), mDistCoef(distCoef.clone()), 
...
#endif
    thread threadLeft(&Frame::ExtractFeatures,this,0,imLeft,0,0);
    thread threadRight(&Frame::ExtractFeatures,this,1,imRight,0,0);
    threadLeft.join();
    threadRight.join();
#ifdef REGISTER_TIMES
...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, 
             FeatureExtractor* extractor, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, 
             const float &bf, const float &thDepth, GeometricCamera* pCamera, Frame* pPrevF, 
             const IMU::Calib &ImuCalib)
    :mpcpi(NULL),mpORBvocabulary(voc),mpFeatureExtractorLeft(extractor),
     mpFeatureExtractorRight(static_cast<FeatureExtractor*>(NULL)),
     mTimeStamp(timeStamp), mK(K.clone()), mK_(Converter::toMatrix3f(K)),mDistCoef(distCoef.clone()), 
     mbf(bf), mThDepth(thDepth),
     mImuCalib(ImuCalib), mpImuPreintegrated(NULL), mpPrevFrame(pPrevF), 
     mpImuPreintegratedFrame(NULL), mpReferenceKF(static_cast<KeyFrame*>(NULL)), 
...
Frame::Frame(const cv::Mat &imGray, const double &timeStamp, FeatureExtractor* extractor,
     ...
     mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    void Frame::ExtractFeatures(int flag, const cv::Mat &im, const int x0, const int x1)
    {
        vector<int> vLapping = {x0,x1};
        if(flag==0)
            monoLeft = (*mpFeatureExtractorLeft)(im,cv::Mat(),mvKeys,mDescriptors,vLapping);
        else
            monoRight = (*mpFeatureExtractorRight)(im,cv::Mat(),mvKeysRight,mDescriptorsRight,vLapping);
    }
...
void Frame::ComputeStereoMatches()
{
    ...
    const int nRows = mpFeatureExtractorLeft->GetImagePyramid()[0].rows;
    ...
    for(int iL=0; iL<N; iL++)
    {
        ...
        if(bestDist<thOrbDist)
        {
            ...
            const int w = 5;
            cv::Mat IL = mpFeatureExtractorLeft->GetImagePyramid()[kpL.octave].rowRange(scaledvL-w,scaledvL+w+1).colRange(scaleduL-w,scaleduL+w+1);
            ...
            if(iniu<0 || endu >= mpFeatureExtractorRight->GetImagePyramid()[kpL.octave].cols)
                continue;

            for(int incR=-L; incR<=+L; incR++)
            {
                cv::Mat IR = mpFeatureExtractorRight->GetImagePyramid()[kpL.octave].rowRange(scaledvL-w,scaledvL+w+1).colRange(scaleduR0+incR-w,scaleduR0+incR+w+1);
